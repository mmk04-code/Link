...
function FreelancerDashboard() {
  ...
  const fetchDashboardData = useCallback(async (token) => {
    try {
      ...
      const profileFullName = (profileRes.data?.full_name || '').trim();
      if (profileFullName) {
        setFullName(profileFullName);
        localStorage.setItem('full_name', profileFullName);
      }
  ...
  const prettyUsername = rawUsername.includes('@') ? '' : rawUsername;
  const displayName = fullName || firstName || prettyUsername || 'there';
